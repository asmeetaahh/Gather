...
## 3g. User dashboard tests (Phase 8)

Rules under test: PRD §10 (DSH-01…05) and the implementation decision **D-073** (composition; the one missing
endpoint, `GET /api/me/draws`; what honestly stays unbuilt). Subscription/charity/winnings LOGIC is already
proven in their own phases (4/5/7) and is only re-exercised here at the rendering level, not re-implemented.
**Total 29 new tests** (1 shared + 15 API + 14 web (1 new file, `dashboard.test.tsx`)); no new migration.

| Layer  | File                                               | Tests | What it proves                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------ | -------------------------------------------------- | ----: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shared | `packages/shared/src/draws.test.ts` (extended)     |     1 | `API_MY_DRAWS_PATH` is `/api/me/draws`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| API    | `apps/api/src/draws/repository.test.ts` (extended) |     5 | `listMyParticipation`: scopes by `user_id` AND filters `draws.status = 'published'` in the same query (the exact PostgREST filter sent); sorts newest month first (client-side, mirroring `findById`'s own tier-result sort — no cross-table PostgREST order relied on); empty when the user has no published-draw entries; a query error or a malformed row is a plain failure, never a silent empty list                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| API    | `apps/api/src/draws/service.test.ts` (extended)    |     4 | `listMine`: empty for a user who never entered a published draw; a real create→simulate→publish draw shows the winning numbers and the caller's own match count; a draft/simulated (unpublished) draw with a candidate entry is invisible even to the user who holds that entry (D-050, re-verified from the dashboard's own read path); two different callers each see only their own entry for the same shared draw                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| API    | `apps/api/src/draws/routes.test.ts` (extended)     |     5 | **Through the real Express app:** `GET /api/me/draws` — 401 with no/forged token, 503 when unwired; empty for a user with nothing; the full lifecycle over HTTP (create → simulate → publish as admin) confirms the candidate draw is invisible before publish and each of two users sees only their own match count after                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Web    | `apps/web/src/dashboard.test.tsx`                  |    14 | **Loading/empty/error, independently per section (item 6):** the dashboard heading appears once identity is confirmed; every section shows an honest empty state with no data at all; one section's failure (e.g. `billingFail`) never blanks an unrelated section. **Real API data (item 9):** subscription status/plan/renewal, charity name/percentage, draw participation with match count, and winnings with prize amount and status are all rendered from the real shared DTOs the fake API returns — not fabricated. **Cross-user isolation:** signed in as Alice, with Bob's subscription/charity/scores/draws/winnings also seeded, none of Bob's data ever appears; each of Alice's five sections shows only her own (empty) state. **Score entry/edit/delete (DSH-02):** add (posts exactly `{playedOn, stablefordScore}`); an out-of-range value is rejected client-side with **no request sent** (the server remains authoritative regardless, item 9 — this is only a faster rejection); the server's `replacedPlayedOn` outcome is shown in words after a sixth score, and the replaced row disappears; edit in place (`PUT /api/scores/:playedOn`); delete after a confirmation dialog, and declining the confirmation sends no request at all |

**Where the rules live, and what that means.** `GET /api/me/draws` is a pure read with no new SQL: the
repository explicitly re-applies the same `status = 'published'` filter the `draw_entries_select_own_published`
RLS policy already enforces for a direct browser read (D-050) — the same "the repository re-applies what RLS
would have applied" pattern `charities/repository.ts` (listed-only) and `winners/repository.ts` (owner-only)
already use. The dashboard itself contains no business logic beyond simple client-side pre-validation that
mirrors the shared `playedOnError`/`stablefordScoreError` functions already used by the API (never a second,
divergent copy of a rule) — the server re-validates every request regardless (item 9). `useMyData`
(`apps/web/src/lib/useMyData.ts`) is the one small piece of shared frontend infrastructure this phase adds: a
loading/error/ready hook so five independent sections don't need five copies of the same guarded-effect
boilerplate; it contains no business rule, only fetch/reload plumbing.

**Mutation-checked:** none this phase — Phase 8 added no new invariant-enforcing code (no SQL function, no
guard trigger, no new authorization branch beyond the pre-existing `requireAuth` already covered by Phase
2/4/5/7's own mutation checks); `GET /api/me/draws` is mounted with `requireAuth` exactly like every other
`/api/me/*` route, and its only new invariant (the `published`-only filter) is covered by a dedicated test
(`never includes a draft or simulated (unpublished) draw, even with a candidate entry (D-050)`) rather than a
mutation check, consistent with how the read-only charity/winners list filters were tested in their own phases.

**Not proven — no hosted run was performed this phase**, and no Stripe-configured run either: the subscription
card's "not subscribed yet" / real-plan states are proven with the fake API only (Stripe's own real behaviour is
Phase 5's concern, unaffected by this phase). The dashboard has not been exercised in a real browser against a
hosted project, and only Chrome-class jsdom rendering (no visual/CSS regression testing) was used. Mobile-width
CSS was written by hand (`apps/web/src/styles.css`'s `.dashboard-grid` uses `auto-fit`/`minmax`, no fixed pixel
breakpoint) but not verified in a real device or browser viewport — only that the markup renders correctly.
