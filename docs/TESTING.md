...
| Winner       | payment state moves Pending → Paid only via allowed transitions            | DRW-12, D-022                                                            |
...
- **Draw workflow:** simulate → publish → winners are implemented and tested against the real database (Section
  ...
  Still planned: real Supabase Auth tokens end-to-end, and the downstream proof-approval → payout steps.
- **Storage:** signed-URL flows for proof; type and size limits.
